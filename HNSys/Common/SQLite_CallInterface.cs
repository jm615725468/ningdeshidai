...
namespace HNSys
{
    public static class SQLite_CallInterface
    {
        ...
        /// <param name="dv"></param>
        /// <returns></returns>
        public static bool GetDataTable(string SQLString, DataGridView dv)
        {
            using (SQLiteConnection dbConn = new SQLiteConnection(ConStr))
            {
                try
                {
                    dbConn.Open();
                    using (SQLiteDataAdapter da = new SQLiteDataAdapter(SQLString, dbConn))
                    {
