...
namespace HNSys
{
    public partial class FrmReport : Form
    {
        ...
        private void FrmReport_Load(object sender, EventArgs e)
        {
            ...
            reportTypes.Add(new ReportType { Display = "停机报警报表", Value = "1" });
            reportTypes.Add(new ReportType { Display = "参数报表", Value = "2" });
            reportTypes.Add(new ReportType { Display = "过辊速度报表", Value = "3" });
            reportTypes.Add(new ReportType { Display = "提示报警报表", Value = "4" });

            this.cmb_ReportType.DataSource = reportTypes;
            this.cmb_ReportType.DisplayMember = "Display";	//显示内容对应List对象中的列名
            this.cmb_ReportType.ValueMember = "Value";	//值对应List对象中的列名
            this.cmb_ReportType.SelectedIndex = 0;
            this.cmb_ReportType.DropDownStyle = ComboBoxStyle.DropDownList; //禁止修改

            InitialDeviceAlarmDataGridView();
        ...
        {
            switch (cmb_ReportType.Text.ToString())
            {
                case "参数报表":
        ...
        private void InitialDeviceParmDataGridView()
        {
            ...
            this.dgv.Columns[0].Width = 250;
            dgv.Columns[0].DataPropertyName = "CellectTime";
            dgv.Columns[1].DataPropertyName = "Operator";
            dgv.Columns[2].DataPropertyName = "DeviceCode";

            for (int i = 0; i < 310; i++)
            {           
                dgv.Columns[i + 3].DataPropertyName = "Parameter" + (i + 1).ToString();
            }
        ...
        #region 报警表初始化
        private void InitialDeviceAlarmDataGridView()
        {
            this.dgv.AutoGenerateColumns = false;
         

            dicDeviceAlarm.Clear();
            dgv.Columns.Clear();

            //额外添加
            dicDeviceAlarm.Add("AlarmTime", "日期时间");
            dicDeviceAlarm.Add("AlarmInfo", "报警名称");
            dicDeviceAlarm.Add("AlarmState", "报警状态");
            dicDeviceAlarm.Add("AlarmID", "报警地址");



            foreach (var item in dicDeviceAlarm.Keys)
            {
                DataGridViewTextBoxColumn dgvc = new DataGridViewTextBoxColumn();
                dgvc.HeaderText = dicDeviceAlarm[item];
                dgvc.ReadOnly = true;
                dgvc.Width = 150;
                dgvc.SortMode = System.Windows.Forms.DataGridViewColumnSortMode.NotSortable;
                this.dgv.Columns.Add(dgvc);
            ...
            this.dgv.Columns[0].Width = 250;
            dgv.Columns[0].DataPropertyName = "AlarmTime";
            dgv.Columns[1].DataPropertyName = "AlarmInfo";
            dgv.Columns[2].DataPropertyName = "AlarmState";
            dgv.Columns[3].DataPropertyName = "AlarmID";
            dicDeviceAlarm.Remove("AlarmTime");
        ...
        private void InitialDeviceOverRollataGridView()
        {
           ...
           dicOverRoll.Add("CellectTime", "采集时间");
            ...
            for (int i = 0; i < 168; i++)
            {
                dgv.Columns[i + 1].DataPropertyName = "Parameter" + (i + 1).ToString();
            }
        ...
        public void StopAlarmReport()
        {
            DateTime t1 = Convert.ToDateTime(dtp_Start.Text);
            DateTime t2 = Convert.ToDateTime(dtp_End.Text);
            ...
            if (ts.Days > 10)
            {
                MessageBox.Show("查询间隔范围太大【10天】！", "查询提示");
            }
            ...
            StringBuilder sb = new StringBuilder();
            sb.Append("select AlarmTime,AlarmInfo,AlarmState,AlarmID");
            sb.Append(" from Alarm where AlarmTime > " + "'" + dtp_Start.Text + "'" + "and AlarmTime <" + "'" + dtp_End.Text + "'" + "order by AlarmTime desc");
        ...
        public void TipAlarmReport()
        {
            ...
            DataTable dt = SQLite.AlarmTipGetDataSet(sb.ToString()).Tables[0];
        ...
        for (int i = 0; i < 312; i++)
        {
            ...
            sb.Append(" from table1 where CellectTime > " + "'" + dtp_Start.Text + "'"+ "and CellectTime <" + "'" + dtp_End.Text + "'"+ "order by CellectTime desc"); 
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            commonMean.dataGridViewToCSV(dgv);
        }
