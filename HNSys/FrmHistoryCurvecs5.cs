...
namespace HNSys
{
    public partial class FrmHistoryCurvecs5 : Form
    {
        ...
        private void InitSet()
        {
            ...
            string 时间范围 = IniHelper.ReadIniData("FrmHistoryCurvecs5_Set", "时间范围", "", SetSavePath);
            if (时间范围 != "")
            {
                numericUpDown1.Value = decimal.Parse(时间范围);
            }

            string Y轴最大值 = IniHelper.ReadIniData("FrmHistoryCurvecs5_Set", "Y轴最大值", "", SetSavePath);
            if (Y轴最大值 != "")
            {
                numericUpDown3.Value = decimal.Parse(Y轴最大值);
                Ymax = int.Parse(Y轴最大值);
            }
            string Y轴最小值 = IniHelper.ReadIniData("FrmHistoryCurvecs5_Set", "Y轴最小值", "", SetSavePath);
            if (Y轴最小值 != "")
            {
                numericUpDown2.Value = decimal.Parse(Y轴最小值);
                Ymin = int.Parse(Y轴最小值);
            }
            string 调整范围 = IniHelper.ReadIniData("FrmHistoryCurvecs5_Set", "调整范围", "", SetSavePath);
            if (调整范围 != "")
            {
                    ...
                    if (dt1.Rows.Count > 0)
                    {
                        this.BeginInvoke((EventHandler)delegate
                        {
                            ...
                            for (int ChartSum1 = 0; ChartSum1 < dt1.Rows.Count - 1; ChartSum1++)
                            {
                                ...
                                {
                                    chart1.Series[6].Points.AddXY(Parmeter.ToOADate(), float.Parse(dt1.Rows[ChartSum1][17].ToString()));
                                }

                                if (checkBox8.CheckState == CheckState.Checked)
                                {
                                    chart1.Series[7].Points.AddXY(Parmeter.ToOADate(), float.Parse(dt1.Rows[ChartSum1][18].ToString()));
                                }

                                if (checkBox9.CheckState == CheckState.Checked)
                                {
                                    chart1.Series[8].Points.AddXY(Parmeter.ToOADate(), float.Parse(dt1.Rows[ChartSum1][19].ToString()));
                                }

                                if (checkBox10.CheckState == CheckState.Checked)
                                {
                                    chart1.Series[9].Points.AddXY(Parmeter.ToOADate(), float.Parse(dt1.Rows[ChartSum1][20].ToString()));
                                }

                                if (checkBox11.CheckState == CheckState.Checked)
                                {
                                    chart1.Series[10].Points.AddXY(Parmeter.ToOADate(), float.Parse(dt1.Rows[ChartSum1][21].ToString()));
                                }

                                if (checkBox12.CheckState == CheckState.Checked)
                                {
                                    chart1.Series[11].Points.AddXY(Parmeter.ToOADate(), float.Parse(dt1.Rows[ChartSum1][22].ToString()));
                                }
        ...
        private void numericUpDown3_ValueChanged(object sender, EventArgs e)
        {
            ...
            string SetSavePath = Application.StartupPath + "\\HNSet\\SetSave.ini";
            IniHelper.WriteIniData("FrmHistoryCurvecs5_Set", "Y轴最大值", numericUpDown3.Value.ToString(), SetSavePath);
        }
        ...
        private void numericUpDown2_ValueChanged(object sender, EventArgs e)
        {
            ...
            string SetSavePath = Application.StartupPath + "\\HNSet\\SetSave.ini";
            IniHelper.WriteIniData("FrmHistoryCurvecs5_Set", "Y轴最小值", numericUpDown2.Value.ToString(), SetSavePath);
        }

        private void numericUpDown1_ValueChanged(object sender, EventArgs e)
        {
            string SetSavePath = Application.StartupPath + "\\HNSet\\SetSave.ini";
            IniHelper.WriteIniData("FrmHistoryCurvecs5_Set", "时间范围", numericUpDown1.Value.ToString(), SetSavePath);
        }

        private void numericUpDown4_ValueChanged(object sender, EventArgs e)
        {
            string SetSavePath = Application.StartupPath + "\\HNSet\\SetSave.ini";
            IniHelper.WriteIniData("FrmHistoryCurvecs5_Set", "调整范围", numericUpDown4.Value.ToString(), SetSavePath);
        }
